...
class Detector
{
    ...
    {
        # remove soft hyphen
        $inputText = preg_replace('~\x{00AD}~u', '', $inputText);

        $allMatches = [];
        $inputTextSymbolArray = mb_str_split($inputText);
        foreach ($targetWords as $targetWord) {
            array_push($allMatches, ...$this->checkForWord($inputTextSymbolArray, $targetWord));
        }
        return $allMatches;
    }
    ...
    private function hasWordAtStart(array $symbols, string $word): bool
    {
        ...
        $textChar = $symbols[0];
        $wordCharLower = mb_strtolower($word[0]);
        $wordCharUpper = mb_strtoupper($word[0]);
        $firstCharMatches =
                ($textChar === $wordCharLower) || (isset(self::CHAR_MAP[$wordCharLower]) && in_array($textChar, self::CHAR_MAP[$wordCharLower], true)) ||
                ($textChar === $wordCharUpper) || (isset(self::CHAR_MAP[$wordCharUpper]) && in_array($textChar, self::CHAR_MAP[$wordCharUpper], true));

        return $firstCharMatches && $this->hasWordAtStart(array_slice($symbols, 1), mb_substr($word, 1));
    }
